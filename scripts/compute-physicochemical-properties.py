...
for rec in SeqIO.parse(fasta_file, "fasta"):
    ...
    if re.search(r"\[SIGNALP\]", description):
        continue
    ...
    if invalid:
        print(
            f"Skipping {header}: invalid amino acid(s): {','.join(invalid)}",
            file=sys.stderr
        )
        continue
    ...
    # modlamp descriptors
    desc = GlobalDescriptor(np.array([seq]))
    desc.calculate_all()
    ...
    print(
        f"{header}\t{seq}\t{len(seq)}\t"
        f"{mol_weight:.2f}\t{net_charge:.2f}\t{pI:.2f}\t"
        f"{hydrophobicity:.2f}\t{amphipathicity:.2f}\t{boman_index:.2f}\t"
        f"{ss[0]:.2f}\t{ss[2]:.2f}\t{ss[1]:.2f}"
    )

...
if __name__ == "__main__":
    ...
    for entry, fragments in frag_dict.items():
        ...
        fragments.sort(key=lambda x: x["cut_site"])
        ...
        for i, frag in enumerate(fragments):
            ...
            if i == 0:
                ...
                if frag_before:
                    print(
                        f">{entry}\tSITE=[{frag['site']}]\tCUT_SITE=[{start}]\tPROBABILITY=[{frag['probability']}]\tSCORE=[{frag['score']}]"
                    )
                    print(frag_before)

            end = fragments[i + 1]["cut_site"] if i + 1 < len(fragments) else full_len
